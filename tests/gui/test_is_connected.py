...
class TestAppIsConnected:
    """Test UI app when is in idle state."""

    def test_is_connected(self, connection_widgets):
        """Test if connection text status is Idle"""
        assert connection_widgets._is_connected.text() == 'Connected'

    def test_port_is_enabled(self, connection_widgets):
        """Test if port widget modification are enabled."""
        assert connection_widgets.server_port.isEnabled() is False
    ...
    def test_sender_mode_enabled(self, connection_widgets):
        ...
        assert sender_mode.isEnabled() is False
    ...
    @pytest.mark.skip(reason='wait a minute bro')
