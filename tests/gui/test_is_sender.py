...
@pytest.mark.usefixtures('sender_mode')
class TestIsSenderMode:
    """Test UI app when is in idle state."""

    def test_is_idle(self, connection_widgets):
        """Test if connection text status is Idle"""
        assert connection_widgets._is_connected.text() == 'Ready to send'
    ...
    def test_ip_entry_readonly(self, connection_widgets):
        ...
        assert local_ip.isReadOnly() is False
    ...
    def test_sender_mode_active(self, connection_widgets):
        """Test if receive radio button widget is toggled."""
        sender = connection_widgets.sender_mode
        assert sender.isChecked() is True

    @pytest.mark.skip(reason="need to check better")
    ...
    def test_connect_btn(self, main_window: MainWindowWidget):
        ...
        assert connect_btn.isEnabled() is False
